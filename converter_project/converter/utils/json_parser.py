...
def parse_payment_methods(response_text: str):
    ...
    return PaymentMethod.objects.filter(currency=currency)


def get_offers_from_json(response_text: str) -> List[Offer]:
    json_array = json.load(open(response_text))
    # json_array = json.loads(response_text)
    raw_offers = json_array['data']
    ...
    for raw_offer in raw_offers:
        seller: Seller = Seller(
            ...
            is_merchant=(
                True if raw_offer['advertiser']['userType'] == 'merchant' else False),
        ...
        offer = Offer(
            ...
            seller=seller,
            trade_type=(
                TradeType.BUY if raw_offer['adv']['tradeType'] == 'BUY' else TradeType.SELL),
